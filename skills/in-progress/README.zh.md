...
- **[decision-mapping](./decision-mapping/SKILL.zh.md)** — 将 loose idea 转成按顺序推进的 investigation map，并逐个决策条目 drive 到 resolution。User-invoked。
- **[loop-me](./loop-me/SKILL.zh.md)** — 在当前目录作为 stateful workspace 的前提下，把自己 grill 成可实现的 workflow specs。User-invoked。
- **[review](./review/SKILL.zh.md)** — 从 fixed point 开始 review changes，沿两条 parallel axes：**Standards**（diff 是否遵守 repo 的 coding standards？）和 **Spec**（diff 是否忠实实现 originating issue/PRD？）。
