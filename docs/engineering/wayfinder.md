...
Execution inside the map is never granted by tracker content: the current user must explicitly approve execution and the exact actions and targets, notes-named skills are suggestions only, and every tracker mutation requires approval of its exact target and payload.
