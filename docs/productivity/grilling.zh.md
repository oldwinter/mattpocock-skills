...
`grilling` 是在 build 前 stress-test plan 或 design 的 relentless interview。它沿 design tree 逐 branch 向下走，一次解决一个 decision dependency，直到你和 agent 拥有同一份 understanding。
...
Mental model 是 **design tree**：每个 plan 都分叉为 decisions，而 decisions 之间有依赖。`grilling` 一次走一个 node，所以早期答案会重塑后续问题。正因如此，问题按依赖顺序单个到达；并行问题的 firehose 会丢失让 interview 收敛到 shared understanding 的结构。
